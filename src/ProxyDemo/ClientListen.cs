    ...
    public class ClientListen
    {
        ...
        private void bindTcpClient(TcpClient readClient, TcpClient writeClient)
        {
            try
            {
                ...
                RelationTcp romrRelation = new RelationTcp() { readTcp = readClient, readSs = readSs, writeTcp = writeClient, writeSs = writeSs, buffer = new byte[1452] };
                RelationTcp toRelation = new RelationTcp() { readTcp = writeClient, readSs = writeSs, writeTcp = readClient, writeSs = readSs, buffer = new byte[1452] };
